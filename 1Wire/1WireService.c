...
void handleDS18B20_TemperatureRequest(MessageData p_msgData)
{
	...
	enqueue(&sensorsToRead, p_msgData.ds1820SensorNumber);
...
{
	while(!isEmpty(sensorsToRead))
	{
	uint8_t g_sensorNumber = front(sensorsToRead);
	dequeue(&sensorsToRead);
